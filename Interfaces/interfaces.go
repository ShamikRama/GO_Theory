...
var i interface{} // выделилась память но все равно nil
i = 42
i = "hello"
...
// Создать не nil интерфес нельзя, только можно присвоить

// В Go интерфейсы — это специальные структуры данных, которые состоят из двух компонентов:

// Тип (Type): Указатель на тип значения, которое хранится в интерфейсе.

// Значение (Value): Указатель на само значение.

// Когда вы объявляете переменную интерфейса, Go выделяет память для этих двух компонентов. Однако, если вы не присваиваете интерфейсу никакого значения, оба этих компонента остаются nil.

// Лучше использовать интерфесы по месту его использования